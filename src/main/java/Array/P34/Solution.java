		...
		while(x <= y){  //y为nums.length-1的时候，必须带上=号，不然数组只有一个元素时将不会进入while循环
			...
			if(nums[mid] > target){     //所求在左边
				...
				if(mid == nums.length - 1 || nums[mid+1] > target){     //1.mid在数组末尾的情况；2.mid右边元素大于target的情况。可以确定mid就是右边界
					res[1] = mid;
					break;
				} else{ //nums[mid+1] == target         //因为升序，nums[mid+1]若不大于target，则必定=target；此时右边界还在右边。
					x = mid + 1;
				}
			}
		}

		if(res[1] == -1) return res;        //右边界没变，说明没有target元素，直接返回[-1, -1]即可。

		x = 0;
		y = res[1];         //若有target元素，则x置0，y置右边界，继续查找左边界
		while(x <= y){
			int mid = x + (y - x) / 2;
			if(nums[mid] > target){
				y = mid - 1;
