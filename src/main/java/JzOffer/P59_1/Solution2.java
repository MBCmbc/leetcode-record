...
class Solution2 {
	public int[] maxSlidingWindow(int[] nums, int k) {
		if(nums == null || nums.length <= 1) return nums;   //特殊输入提前返回
		...
		for(int i = 0; i < nums.length; i++){   //i可以认为是即将入窗口的元素下标，也就是本轮循环中，窗口最后一个元素的下标。
			while(!queue.isEmpty() && nums[queue.peekLast()] <= nums[i]) queue.pollLast();  // 保证从大到小 如果前面数小则需要依次弹出，直至满足要求
			queue.addLast(i);       //当前元素入队
			if(queue.peek() < i - k + 1) queue.poll();      //队首元素不在窗口内，出队列。在初期未形成窗口时（元素不足k个），此条语句不会进入。后面窗口开始移动时才起作用。

			if(i+1 >= k) res[i+1-k] = nums[queue.peek()];   //窗口最大值放入结果数组。在初期未形成窗口时（元素不足k个），if始终为false，后面窗口形成才为true，进入向结果数组添加元素。
		}
